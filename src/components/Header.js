...
function Featured() {
  return (
    <Link href="/blog/tailwindcss-v3">
      <a className="items-center hidden px-3 py-1 ml-3 text-xs font-medium leading-5 rounded-full text-sky-600 dark:text-sky-400 bg-sky-400/10 xl:flex hover:bg-sky-400/20">
        <strong className="font-semibold">GAM Wiki</strong>
        <svg
          width="2"
          height="2"
          fill="currentColor"
        ...
        <span className="ml-2">Tuyển cộng tác viên soạn bài, nội dung website</span>
        <svg
          width="3"
          height="6"
  ...
  return (
    <div className={clsx(className, display)} {...props}>
      ...
      >
        ...
        <div className="fixed w-full max-w-xs p-6 text-base font-semibold bg-white rounded-lg shadow-lg top-4 right-4 text-slate-900 dark:bg-slate-800 dark:text-slate-400 dark:highlight-white/5">
          <button
            type="button"
        ...
        <div className="mx-auto max-w-8xl">
          ...
          >
            <div className="relative flex items-center">
              <Link href="/">
                <a
                  className="flex-none mr-3 overflow-hidden md:w-auto"
                  onContextMenu={(e) => {
                    e.preventDefault()
                    Router.push('/brand')
                  }}
              ...
              <SearchButton className="flex items-center justify-center w-8 h-8 ml-auto -my-1 text-slate-500 hover:text-slate-600 lg:hidden dark:text-slate-400 dark:hover:text-slate-300">
